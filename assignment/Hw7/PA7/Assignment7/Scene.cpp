...
Vector3f Scene::castRay(const Ray &ray, int depth) const {
    ...
    // ray hit an object, perform path tracing on hit (shading) point
        ...
        if (dist_dir < 0.01) {
            ...
            L_dir = inter_light.emit * m->eval(ws, wo, N) * dotProduct(ws, N) * dotProduct(-ws, NN) / dotProduct(ws_unorm, ws_unorm) / pdf_light;
        }
        ...
        if (p_RR < RussianRoulette) {
            ...
            L_indir = indir_shade_color * m->eval(wi, wo, N) * dotProduct(wi, N) /
