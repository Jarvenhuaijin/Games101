...
inline bool Bounds3::IntersectP(const Ray& ray, const Vector3f& invDir, const std::array<int, 3>& dirIsNeg) const {
    ...
    // shuffle min & max
    Vector3f t_min(std::min(t_pmin.x, t_pmax.x), std::min(t_pmin.y, t_pmax.y), std::min(t_pmin.z, t_pmax.z));
    Vector3f t_max(std::max(t_pmin.x, t_pmax.x), std::max(t_pmin.y, t_pmax.y), std::max(t_pmin.z, t_pmax.z));
